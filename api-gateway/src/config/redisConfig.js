 ...
 *
 * This is needed because RedisStore (from rate-limit-redis) expects
 * a .call() method which ioredis has but Upstash doesn't.
 */
class UpstashRedisAdapter {
  ...
  async call(...args) {
    try {
      ...
      if (cmd === "incr") return await this.incr(params[0]);
      if (cmd === "incrby")
        return await this.incrby(params[0], Number(params[1] || 1));
      if (cmd === "decr") return await this.decr(params[0]);
      if (cmd === "get") return await this.get(params[0]);
      if (cmd === "set") return await this.set(params[0], params[1]);
      if (cmd === "setex")
      ...
      if (cmd === "ttl") return await this.ttl(params[0]);
      if (cmd === "pttl") return await this.pttl(params[0]);
      if (cmd === "del") return await this.del(params);
      if (cmd === "exists") return await this.exists(params[0]);
      if (cmd === "flushdb") return await this.flushdb();
      ...
      if (cmd === "script") {
        ...
        if (subcommand === "exists") return [1];
        if (subcommand === "flush") return "OK";
      }
 ...
 *   - Connection: Cloud-based via HTTPS
 *   - Best for: Testing in production-like environment
 *
 * TOGGLE: Set USE_UPSTASH_REDIS=true in .env to switch
 */
